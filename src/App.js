...
function App(props) {
  ...
  const onBtnCheckout = () => {
    ...
    props.firebase.updateType(
      tabState == 'delivery' ? 'ONLINE_DELIVERY' : 'ONLINE_PICKUP'
    )
            ...
            <Button
              ...
              onClick={() => {
                ...
                window.location.assign(
                  'https://www.ubereats.com/en-US/chicago/food-delivery/hello-jasmine/JnCl5_dnTQatU1lwEWUewg/'
                )
              }}
              color="inherit"
              variant="contained"
  ...
  const RenderTabBar = () => (
    ...
    >
      <AppBar
        position="static"
        color="default"
        style={{ marginTop: (width / 50) * (1 - mobileFlag) }}
      >
        <Tabs
          value={value}
      ...
      <CardActionArea>
        ...
        >
          <Grid item md={5} xs={5} style={{ textAlign: 'center' }}>
            <CardMedia
              style={{ height: 140, width: width / 3, maxWidth: 120 }}
    ...
    >
      ...
      {items.map((item, id) =>
        item.Category == cates[value].Category ? (
          <Grid key={id} item lg={4} md={4} sm={6} xs={12}>
  ...
  const RenderItemModifiers = props => (
    <div style={{ paddingTop: 382 - mobileFlag * 100 }}>
  ...
  const RenderItemDetWebFooter = () => (
    <div>
      {editDetDlg ? (
        ...
        <DialogActions style={{ backgroundColor: 'white' }}>
          <Grid container direction="row" justify="space-between" space={10}>
            <div className="TopButtons">
              <IconButton
                onClick={() =>
                  selCount > 1 ? setSelCount(selCount - 1) : null
                }
              >
                <i className="material-icons"> remove_circle_outline </i>
              </IconButton>
            ...
            <Button variant="contained" onClick={onAddOneOrder} color="primary">
              Add to Order
            </Button>
          </Grid>
        </DialogActions>
  ...
  const RenderChooseButton = props => {
    ...
    return (
      <Grid container direction="row" justify="center" alignItems="center">
        ...
        <Grid item xs={12} sm={3} className={classes.chooseType}>
          <Grid container direction="row" justify="center" alignItems="center">
            <Button onClick={rightOnClick} variant="contained" color="primary">
  ...
  if (choosePage <= 2)
    return (
      <RenderChoosePage
        height={height}
        page={choosePage}
        getLatLong={props.firebase.getLatLong}
      ></RenderChoosePage>
    )

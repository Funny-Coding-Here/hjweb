...
const AnyReactComponent = ({ text }) => (
  <div className="TopButtons" style={{ width: 300, alignItems: 'center' }}>
    <FontAwesomeIcon icon={faMapMarkerAlt} size="3x" color="#f00" />
    <div style={{ paddingLeft: 15 }}>
      <h2>{text}</h2>
      <p style={{ marginBlockStart: 0, marginBlockEnd: 0, fontSize: 14 }}>
        Bubble tea shop
      </p>
      <p style={{ marginBlockStart: 0, marginBlockEnd: 0, fontSize: 14 }}>
        with Taiwanese fare
...
function Checkout(props) {
  ...
  const onPlaceOrder = async props => {
    ...
    let now = moment(new Date())
      .add(business.Estimate_time_min, 'm')
      .format('LLL')
    console.log('=====================')
    let items = []
    const item_ids = {}
    ...
    hlp
      .post(
        'https://us-central1-infishare-client.cloudfunctions.net/onlineChargeCust',
        param,
        token
      )
      .then(result => {
  ...
  const RenderItemDetWebFooter = () => (
    <DialogActions style={{ backgroundColor: 'white' }}>
      <Button variant="contained" onClick={onUpdateOrder} color="primary">
        Update Order
      </Button>
    </DialogActions>
  )
  ...
  const RenderItemDetHeader = props => (
    ...
    >
      ...
      <p style={{ marginBlockStart: 0 }}>
        Descriptions : {selItem.Description}
      </p>
      {mobileFlag ? null : <RenderItemDetWebFooter />}
    </div>
  )

  const RenderItemModifiers = props => (
    <div style={{ paddingTop: 382 - mobileFlag * 170 }}>
          ...
          if (props.item.info[key][modi]) {
            for (let t in modis[key]['Options']) {
              if (modis[key]['Options'][t].Name == modi) {
                infos.push({
                  name: modi,
                  price: '$' + modis[key]['Options'][t].Price_offset,
                })
              }
            }
          }
    ...
    return (
      <div>
        <div className="FlexView">
          <p
            style={{ fontWeight: 'bold', fontSize: '18px', marginBlockEnd: 0 }}
          >
            {props.item.item.Name}
          </p>
          <p style={{ fontWeight: 'bold', marginBlockEnd: 0 }}>
            ${props.item.item.Price}
          </p>
        </div>
        {infos.map((info, id) => (
          <div key={id} className="FlexView">
  ...
  const RenderContactInfo = () => (
    <div>
      <h3>Contact Info</h3>
      <Grid container direction="row" space={3} justify="space-between">
  ...
  const RenderTipping = () => (
    <div>
      <div className="TopButtons">
        ...
        <p style={{ color: '#777', fontSize: 18, marginLeft: 10 }}>
          {' '}
  ...
  return (
    ...
    >
      ...
      <div
        className="App"
        style={{ paddingLeft: width / 20, paddingRight: width / 20 }}
      >
          ...
          <Grid item md={5} sm={12} xs={12}>
            {mobileFlag ? (
              <div>
                ...
                {orderItems.map((item, id) => (
                  <RenderOrderItem item={item} key={id} id={id} />
                ))}
              </div>
            ) : null}
            <RenderContactInfo />
            ...
            {flagPayment == 'hide' ? (
              <div>
                ...
                <p style={{ color: '#aaa' }}>
                  Complete your contact info to see payment options.
                </p>
              </div>
            ) : (
              <div>
              ...
              <div style={{ margin: width / 60 }}>
                ...
                <div className="TopButtons">
                  ...
                  {business ? (
                    ...
                    >
                      {business.Estimate_time_min - 5}-
                      {business.Estimate_time_min + 5} min Estimated
                    </p>
                  ) : null}
                </div>
                <div className="TopButtons">
                ...
                <div className="FlexView">
                  ...
                  <p style={{ fontSize: 18, marginBlockEnd: 0 }}>
                    ${subtotal.toFixed(2)}
                  </p>
                </div>
                <div className="FlexView">
